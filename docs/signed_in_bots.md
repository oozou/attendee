...
We support signed-in bots for Google Meet. You'll need to create a new Google Workspace account for the bot to use. You'll then configure the workspace so that the Attendee server is an SSO Identity Provider (IdP) that the bot can use to sign in. This is more reliable than signing in with a username and password.

There are two SSO options:
*   **Option A: SAML** — the original approach, requires generating a certificate and private key
*   **Option B: OIDC** (recommended for new setups) — simpler setup, credentials are auto-generated

Both options require creating a Google Workspace account first.
...
#### Create a new SAML Google Meet bot login
...
3. Select "SAML" as the SSO mode.
4. Enter the requested information. For private key and certificate, you can upload the key.pem and cert.pem files you generated previously. The email must match the email of the bot user in your Google Workspace account. The certificate must match the certificate you added when creating the Legacy SSO profile.

### Option B: OIDC SSO (Recommended for new setups)

OIDC is simpler to set up — no certificate generation or `xmlsec1` dependency required. Client credentials are auto-generated by Attendee.

4. Navigate to the Admin Console, then to Security -> Set up single sign-on (SSO) with a third party IdP.
5. On the SSO page, click "Add SSO profile" and select "OIDC".
6. Enter the following information:
    - Client ID — will be provided by Attendee after creating the login (see step 10)
    - Client Secret — will be provided by Attendee after creating the login (see step 10)
    - Issuer URL — `https://app.attendee.dev/bot_sso` (For self-hosted instances, use your hostname)
7. Click the "Save" button.
8. Under the Manage SSO profile assignments section, assign the OIDC profile to the bot user's organizational unit.

#### Create a new OIDC Google Meet bot login
9. Navigate to the Settings -> Credentials page in the Attendee dashboard.
10. Scroll down to the Google Meet Bot Logins section and click the "Add Login" button.
11. Select "OIDC" as the SSO mode.
12. Enter the workspace domain and bot email address.
13. Click "Add Login" — a modal will appear with the auto-generated Client ID and Client Secret. **Copy these now** as the Client Secret will not be shown again.
14. Return to the Google Workspace Admin Console and enter the Client ID and Client Secret from step 13 into the OIDC SSO profile you created in steps 5–7.
